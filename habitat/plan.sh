...
do_download() {
  # GOLANG doesn't allow relative imports so we need to import 'cmd' and 'types'
  # using a fully qualified github path. Simple solution is to symlink /src
  # in correct position in $GOPATH/src/github.com heirarchy
  mkdir -p "$scaffolding_go_workspace_src/github.com/trickyearlobe"
  ln -sf "/src" "$scaffolding_go_workspace_src/github.com/trickyearlobe/a2tool"
  do_default_download
}
...
# your package.
do_install() {
  # The GO scaffold uses the Makefile in do_build() but ignores it in do_install() so fudge it for now
  # Should submit a PR to have do_install call `make install` if we have a Makefile
